...
Alternatively, an analyst may assume ``principal ignorability'' \citep{jo2009use,dingLu}:
...
\begin{equation}
  %\begin{split}
    \heff0\equiv \hat{\beta}_2\mbox{ and }
    \heff1\equiv \hat{\beta}_3-\hat{\beta}_2
%  \end{split}
\end{equation}
are M-estimators, with $\heff0\rightarrow_p\eff0$ and $\heff1\rightarrow_p\eff1$ as $n\rightarrow\infty$
\end{prop}
...
\item Estimate $\heff0=\hat{\beta}_2$ and $\heff1=\hat{\beta}_3-\hat{\beta}_2$ with sampling variance \eqref{eq:sandwich}
\end{enumerate}
...
By randomization, (Assumption \ref{ass:rand}), $\EE[\bxy|\st]=\EE[\bxy|\st,Z=0]=\EE[\bxy|\st,Z=1]$, so 
estimates of principal-stratum-specific means of $\bxy$ can be extrapolated from the treatment to the control group.
...
\begin{prop}\label{prop:interactions}
  ...
  \begin{equation}\label{eq:interaction}
    \begin{split}
    Y_i=&\beta_0+\beta_1\sti+\beta_2 Z_i+\beta_3Z_i\sti\\
    &+\bm{\gamma_1}'\bxy_i+\bm{\gamma_2}'\bxy_i\sti+
    \bm{\gamma_3}'\bxy Z_i+\bm{\gamma_4}'\bxy_i Z_i\sti+\epsilon_i
    \end{split}
  \end{equation}
  with $\EE[\epsilon_i|\bxi,Z_i]=\EE[\epsilon_i|\bxi,Z_i,\sti]=0$ and $\epsilon_i\independent\epsilon_j$ when $i\ne j$.
...
In other words, the only interactions between covariates and treatment assignment are due to differences between principal effects---a fairly strong assumption.
...
%   \end{split}
% \end{equation}
with $\beta_3=0.3-\beta_1$. Hence, $\beta_0=\beta_2=\gamma_4=0$, when $\gamma_3=0$ (i.e. no interaction between $Z$ and covariates), there is no average treatment effect in the $\st=0$ stratum, the coefficient for the unobserved covariate $x_3$ is constant across treatment groups and principal strata, and we do not consider three-way interactions between $x_1$ or $x_2$, $\st$, and $Z$.
...
For the Bayesian mixture model and GEEPERs we estimated treatment effects as $\heff0=\hat{\beta}_2$ and $\heff1=\hat{\beta}_2+\hat{\beta}_3$.
...
Since the PSW estimate did not use an outcome model, we estimated $\heff0=\hat{\mut0}-\hat{\muc0}$ and $\heff1=\hat{\mut1}-\hat{\muc1}$.
