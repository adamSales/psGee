...
While such a setup may represent the simpliest and easiest principal stratification problem, we believe it is a common scenario.
...
We begin by introducing an assumption that we term ``Covariate Ignorability,'' or CI. It is a slighty weaker version of \citet{jiangDing2021}'s ``auxilliary indpendence'' assumption.
...
%      +\beta_3Z_i+\beta_4\sti Z_i+\bm{\beta_5}'\bxy_i\sti+
 %   \bm{\beta_6}'\bxy Z_i+\bm{\beta_7}'\bxy_i Z_i\sti+
  \end{equation}
  with $\EE[\epsilon_i]=0$, $\{Z_i,\sti,\bxy_i\}\independent \epsilon_i$.
  Say $\ppi$ linearly independent of $\bxy$, and
  that the following model is fit with OLS
...
Such a model relies on what \citet{bo2002} calls ``Additivity of Treatment Assignment Effect,'' that $\bm{\beta_1}^{C}=\bm{\beta_1}^{T}$.
...
Finally, potential outcomes were generated as in \eqref{eq:interaction}, with $\bxy=[\bm{x_1},\bm{x_2},\bm{x_3}]$,
and with $\EE[\epsilon_i]=0$ and $Var(\epsilon_i)=1/2$.
Across simulation runs, intercepts $\beta_0^C=\beta_0^T=0$ and the coefficient on the unobserved covariate $x_3$, $\beta_{1,3}^C=\beta_{1,3}^T= 1/\sqrt{6}$.
The distribution of $\epsilon_i$ and other coefficients $\bm{\beta}$ were manipulated factors in the study.
