...
%\usepackage{endfloat}% To `separate' figures and tables from text if required
...
Parametric mixture models are often difficult for applied researchers to implement---it is far removed from the regression models that dominate quantiative social science.
Fitting them may require familiarity with Bayesian Markov Chain Monte Carlo methods or liklihood optimization routines---even when these are automated \citep[e.g.]{PStrata}, they are slow and require an additional round of diagnostics for model convergence.
Furthermore, the usual model checking techniques of regression analysis, such as examining residuals, are not straightforward to apply in these contexts.
Combined with their fragility and heavy dependence on model specification, these factors have severely limited their accessibility to applied researchers. 
...
A separate set of results in \citet{jo2002}, \citet{ding2011},
\citet{jiangDing2021}, \citet{richardson2023estimating}, and others have shown that, under certain circumstances, researchers can use baseline covariates to identify and estimate principal effects without relying on principal ignorability.
...
Unlike any principal stratification estimator we are aware of, \geepers estimates are based on widely-used regression techniques, making them easy to compute, check, and understand for researchers without an extensive background in statistics. 
Existing methods fail to meet three needs: (1) accessibility, (2) robustness, and (3) model flexibility. \textsc{geepers} addresses all three.
...
First, how does the performance of the \textsc{geepers} estimator vary under different conditions, including sample size, the extent to which $\bx$ predicts $\st$, %whether and how the principal effects themselves vary, 
...
% \begin{tabular}{r|ll}%{*{3}{c}}
%   \hline
%   Factor &\multicolumn{2}{l}{Levels}\\
%   & Part 1 & Part 2\\
%   \hline
% Sample size per condition $n$ &$100,200,\dots,1000$ & 500, 1000\\
% $\alpha$ & $0,0.2,0.3,\dots,1$ & 0, 0.2, 0.5 \\
% The distribution of $\epsilon_i$& Normal & Normal, Uniform\\
% %$\beta_1$ & \multicolumn{2}{l}{0, 0.3}\\
% \multirow{8}{*}{Interactions in \eqref{eq:y-sim}} &  \multirow{8}{1.5in}{None $\gamma_1=1/\sqrt{6};\gamma_2=\gamma_3=0$}& None\\
% & &$\gamma_1=1/\sqrt{6};\gamma_2=\gamma_3=0$\\ 
% & & $\bm{x}:\st$\\
% && $\gamma_1=3/(4\sqrt{6})$; $\gamma_2=1/(2\sqrt{6})$; $\gamma_3=0$\\
% & & $\bm{x}:Z$\\
% &&$\gamma_1=1/\sqrt{6};\gamma_2=0$; $\gamma_3=1/(2\sqrt{6})$\\
% && $\bm{x}:\st$ \& $\bm{x}:Z$\\
% & & $\gamma_1=3/(4\sqrt{6})$; $\gamma_2=\gamma_3=1/(2\sqrt{6})$\\
...
\begin{center}
  [Table \ref{tab:factor} around here]\\
\end{center}
...
For reference, in the application of Section~\ref{sec:opts}, there were two covariates. In the model for principal scores, their standardized coefficients were roughly 0.46 and 0.62, and the AUC was 0.68. In the application of Section~\ref{sec:fh2t}, the principal score models (which were far more complex than either the simulation model or the model in Section \ref{sec:opts}) had AUCs between 0.745 and 0.795. 
...
In this section, we evaluate estimates of $\eff1$. Results for estimates of $\eff0$, which are similar, can be found in the online appendix. 
In replication $b=1,\dots,500$, define the error of a given estimator $\heff1_b$ as $err_b=\heff1_b-\eff1$. The bias of $\heff1$, $\EE[\heff1-\eff1]$, is estimated as $\widehat{bias}=\sum_b err_b/500$, and the empirical standard error is defined as $\left\{\sum_b(\heff1_b-\overline{\heff1})^2/499\right\}^{1/2}$, the sample standard deviation of $\heff1$. We also report the empirical coverage of confidence intervals, the proportion of replications in which $\eff1$ is within the estimated confidence bounds of $\heff1$. Results on root-mean-squared-error %$\left(\widehat{bias}^2+SE_{emp}^2\right)^{1/2}$ 
...
\begin{center}
  [Figure \ref{fig:alphan} around here]\\
\end{center}
...
Our next set of results compares the performance of \geepers, \pmm,
and \psw when one or more assumptions of each method may be violated.
We present results for $n=500$ here; results from when $n=1,000$,
which are similar, can be found in the online appendix. 

\begin{center}
  [Figure \ref{fig:boxplots} around here]\\
\end{center}
...
When $\alpha=0$, Assumption \ref{ass:vps} is violated; %, standard errors for \geepers were too high to 
...
\begin{center}
  [Table \ref{tab:coverage} around here]\\
\end{center}
...
\subsection{Accounting for Imperfect Compliance: Re-Analysis of the Obstetrics and Periodontal Therapy Study}\label{sec:opts}
\sloppy
...
\begin{center}
  [Figure \ref{fig:opt} around here]\\
\end{center}
...
However, bottom-out hints are unavailable in the other three arms of the study, violating this condition (\citealt{sales2021student} offer a solution, which we find unconvincing).
...
In an alternative analysis, we chose a subset of covariates to include in $\bxs$ %and $\bxy$ 
...
\begin{center}
  [Figure \ref{fig:effects} around here]\\
\end{center}
...
\begin{center}
  [Figure \ref{fig:compare} around here]\\
\end{center}
...
\textsc{geepers} is a straightforward approach to principal effect estimation under strong monotonicity, which is more robust---though sometimes less precise---than alternative approaches under a wide array of scenarios. It is built on widely-used regression models, making it accessible to applied researchers. 
...
We used Grammarly Pro (Version 14.1243.0) and ChatGPT (GPT-4-turbo, current as of July 2025) for editing and language improvement. 
...
$\alpha$ &\multicolumn{2}{l}{$\rd{0},0.2,0.3^\dagger,0.4^\dagger,0.5,0.6^\dagger,0.7^\dagger,0.8^\dagger,0.9^\dagger,1^\dagger$}\\
The distribution of $\epsilon_i$&\multicolumn{2}{l}{ Normal, 
\rd{Uniform}}\\
%$\beta_1$ & \multicolumn{2}{l}{0, 0.3}\\
\multirow{4}{*}{Interactions in \eqref{eq:y-sim}}& None:  $\gamma_1=1/\sqrt{6};\gamma_2=\gamma_3=0$\\ 
& \rd{$\bm{x}:\st$}: $\gamma_1=3/(4\sqrt{6})$; $\gamma_2=1/(2\sqrt{6})$; $\gamma_3=0$\\
& \rd{$\bm{x}:Z$}: $\gamma_1=1/\sqrt{6};\gamma_2=0$; $\gamma_3=1/(2\sqrt{6})$\\
& \rd{$\bm{x}:\st$ \& $\bm{x}:Z$}: $\gamma_1=3/(4\sqrt{6})$; $\gamma_2=\gamma_3=1/(2\sqrt{6})$\\
% between $\st$ and covariates & No, Yes\\
% Interactions between $Z$ and covariates& No, Yes\\
\hline
\multicolumn{3}{p{4.5in}}{\footnotesize Factor levels in red violate assumptions of \geepers, \pmm, or both. Factors fully crossed except where indicated.}\\
