...
bind_rows(
  ...
  mutate(
    ...
    scale_y_continuous(minor_breaks=c(ateDF$estimates,-0.75,0.75))+
...
compare <- bind_rows(
  ...
  stanResults)%>%
    ...
    geom_errorbar(data=ateDF,aes(EFF,ymin=ymin,ymax=ymax),width=0,color='black',inherit.aes=FALSE)+
    geom_hline(yintercept=0)+
    #geom_hline(data=ateDF,aes(yintercept=estimates),linetype="dashed")+
    scale_x_continuous("Principal Stratum",breaks=c(0,.25,.5),minor_breaks=NULL,
                     labels=c("Non-\nBottom-\nOuter","ATE","Bottom-\nOuter"),limits=c(-.15,.65))+
...
tikz("figure/compareMethods.tex",width=5,height=3,standAlone=TRUE)
print(compare)
dev.off()
