...
// producing error
 ...
 YtrtY~normal(alphaT+XtrtY*betaY,sig); // model for outcomes in treatment group


 for(i in 1:nc)
  target += log_sum_exp(
   log(piC[i]) + normal_lpdf(YctlY[i] // I'm not sure what this should
          | alphaCBO+XctlY[i,]*betaY,sig),
   log((1-piC[i])) + normal_lpdf(YctlY[i] |alphaCNBO+XctlY[i,]*betaY,sig));
}
