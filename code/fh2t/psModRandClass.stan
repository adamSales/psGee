...
data{
 ...
 int<lower=1,upper=nclass> classT[nt];
 int<lower=1,upper=nclass> classC[nc];
...
parameters{
 ...
 real classEff[nclass];
 real<lower=0> sigClass;
 ...
 for(i in 1:nc)
  target += log_sum_exp(
   log(piC[i]) + normal_lpdf(YctlY[i] // I'm not sure what this should
          | alphaCBO+XctlY[i,]*betaY+classEff[classC[i]],sigCBO),
   log((1-piC[i])) + normal_lpdf(YctlY[i] |alphaCNBO+XctlY[i,]*betaY+classEff[classC[i]],sigCNBO));
}
