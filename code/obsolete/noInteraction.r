...
Est <- function(Y0,X0,ps,Y1,X1,S1,...){

    ## system of eq's
    ## Yi-(1-Z)*(mu00+p_i*eta0)-Z*(m10+S_i*eta1)-X_i'beta
    ## Zi*(SiYi-mu0*Si-eta1*Si-Xi'beta*Si)
    ## (1-Zi)*(piYi-p_i*mu0-pi^2*eta-p_i*X_i'beta)
    ## X_iY_i-(1-Zi)*(X_i*mu00+X_ip_i*eta0)-Zi*(Xi*mu10+XiSi*eta1)-X_i*X_i'beta

    n <- length(ps)
    A <- lapply(1:n,function(i)
        rbind(
            c(mu00=1,eta=ps[i],X0[i,]),
    ...
    estList <- list(mu00=est['mu00'],eta=est['eta'],beta=est[!names(est)%in%c('mu00','eta')])

    list(est=estList,ef=do.call('rbind',lapply(1:n,function(i) t(B[[i]]-A[[i]]%*%est))))
}

## Y - mu00-eta*ps-x'beta
## psY-ps mu00-eta*ps^2-ps x'beta
## XY-Xmu00-eta*ps*X-XX'beta
...
A21 <- function(smod,X0,Y0,est){
    ...
    rbind(
        t(q)%*%X0tilde,
        -t((Y0-est$mu00-X0%*%est$beta)*q-est$eta*u)%*%X0tilde,
